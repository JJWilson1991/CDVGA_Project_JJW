...
Monthseries19<-ggplot2 :: autoplot(f6_ts[,c( "Gray_Fox","Raccoon")]) +
...
```
```{r}
#create lagged predictors for gf
...
```
```{r}
#pull all the rmse into table for indivialu predictors
...
```{r}
#create araim to rpedict raccoon using incrasing number of gF months into the past 
...
Rac_cases<-ts(f6$Raccoon) #create layer for actual recorded case numbers of raccoons

Raccoon_model_plot<-forecast(object=rfit4tr, h=105, xreg=Fox_lag_19[13:423,1:3], lower = 0, level = c(80, 95) )%>%autoplot() +autolayer(Rac_cases) +theme_classic()+ theme(legend.position = "none") + labs(title="Best fit model for predicting CDV cases in Raccoons", x="Number of months since start of recording period, January 1975", y="Number of cases")#autolayer plots a time series


print(Raccoon_model_plot)

ggsave(here::here("results", "Models","Raccoon_model_plot.png"), plot = Raccoon_model_plot)




Fox_cases<-ts(f6$Gray_Fox)#create layer for gray fox cases

Fox_model_plot<-forecast(object  =gfit2tr, h=105, xreg=Raccoon_lag_19[13:423,1], lower = 0, level = c(80, 95) )%>%autoplot() +autolayer(Fox_cases) +theme_classic()+theme(legend.position = "none") + labs(title="Best fit model for predicting CDV cases in Gray Foxes", x="Number of months since start of recording period, January 1975", y="Number of cases")#autolayer plots a time series


print(Fox_model_plot)

ggsave(here::here("results", "Models","Fox_model_plot.png"), plot = Fox_model_plot)
