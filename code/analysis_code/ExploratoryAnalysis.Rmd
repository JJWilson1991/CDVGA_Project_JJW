...
CDV_Clean_age <- CDV_Clean%>% dplyr :: filter(Age %in% c("Adult", "Subadult", "Juvenile"))
Age_wrap_species <- ggplot(CDV_Clean_age, aes(Age, fill=Age, color=Age)) +geom_bar() + theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8), legend.position = "none") + 
  facet_wrap(~Species)

Age_wrap_species
ggsave(here("results", "Figures", "Age_wrap_species.png"), plot=Age_wrap_species)
```
```{r}
...
RGFAge %>% saveRDS(here("data", "processed_data", "RGFAge"))
...
sexspec<- with(RGFSex, table(Sex, Species))


chisex<- chisq.test(sexspec)
chisex
...
agespec<- with(RGFSex, table(Age, Species))


chiage<- chisq.test(agespec)
chiage
