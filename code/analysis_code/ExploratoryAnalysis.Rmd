...
breedingadj1 <- breeding %>% dplyr :: mutate(lengthSea= c(3,3,6,3,3,6))
 ...
 ggplot(breedingadj2, aes(Season, SeaAdj, fill = Species.x, color = Species.x, group = Species.x)) + geom_point() +geom_line()
```
```{r}
...
ggplot(breeding2adj2, aes(Season, SeaAdj, fill = Sex.x, color = Sex.x, group = Sex.x)) + geom_point() +geom_line() + facet_wrap(Species.x)

season_Spec <-breeding2adj2[,c(1,2,6)] %>% spread(key = Season, value = SeaAdj)
