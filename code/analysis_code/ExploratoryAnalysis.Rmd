...
```
```{r}
AGESE <- with(RGFAge,table(Species, Age))
 AGESE 
Tibage<- as.tibble(AGESE) 
TibageSP <-spread(Tibage,Age,n)
TibageSP %>% mutate(sumrow = rowSums((2,)))#%>% dplyr:: mutate(total= sum("Adult", "Juvenile", "Subadult"))
```
```{r}
...
tb<- left_join(TibageSP,fudge)
tb
tb %>% mutate_at(vars(Adult, Juvenile, Subadult), funs(./Total))
