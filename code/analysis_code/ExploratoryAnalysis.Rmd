...
library(viridis)
ggplot(data = dftr) +  geom_polygon(aes(x = long, y = lat, fill = Freq, group = group), color = "white") + coord_fixed(1.3) +  scale_fill_viridis(option = "B") + theme( 
...
```
```{r}
ggplot(data = cotr) +  geom_polygon(aes(x = long, y = lat, fill = Freq, group = group), color = "white") + coord_fixed(1.3)  + scale_fill_viridis(option="B") + theme_map() + theme( 
  axis.text = element_blank(),
  axis.line = element_blank(),
  axis.ticks = element_blank(),
  panel.border = element_blank(),
