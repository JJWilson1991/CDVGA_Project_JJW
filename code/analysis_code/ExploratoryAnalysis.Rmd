...
```
```{r}
ggplot(data = dftr) + geom_polygon(data = us, aes(x = long, y = lat, group = group), color ="gray50", fill = "white") + geom_polygon(aes(x = long, y = lat, fill = Freq, group = group), color = "gray50") + coord_fixed(1.3) +  scale_fill_gradient(low = "white", high = "red", na.value = "azure", trans = "log10") + theme( 
  axis.text = element_blank(),
  axis.line = element_blank(),
  axis.ticks = element_blank(),
  panel.border = element_blank(),
  panel.grid = element_blank(),
  axis.title = element_blank(), legend.position = "right") +  geom_text(data = SECEnt, aes(x = x, y = y, label = state.abb, group = NULL))
```
...
```
```{r}
ggplot(data = cotr) +  geom_polygon(aes(x = long, y = lat, fill = Freq, group = group), color = "azure4") + coord_fixed(1.3)  + scale_fill_gradient(high = "magenta4", low = "white", na.value="azure", trans = "log10") + theme_map() + theme( 
  axis.text = element_blank(),
  axis.line = element_blank(),
  axis.ticks = element_blank(),
  panel.border = element_blank(),
...
```{r}
CoYr <- with(CDV_Clean, table(County, CollectionYear))
COYR <- as.data.frame(CoYr)
cotry <- left_join(GAC, COYR)
```
```{r}
ggplot(data = cotry) +  geom_polygon(data = GAC, aes(x = long, y = lat, group = group), color ="gray50", fill = "white") + geom_polygon(aes(x = long, y = lat, fill = Freq, group = group), color = "azure4") + coord_fixed(1.3)  + scale_fill_gradient(high = "red", low = "white", na.value="azure") + theme_map() + theme( 
  axis.text = element_blank(),
  axis.line = element_blank(),
  axis.ticks = element_blank(),
  panel.border = element_blank(),
...
```
```{r}
CoSPE <- with(raccoongrayfox, table(County, Species))
COSPE <- as.data.frame(CoSPE)
COSPEC <- left_join(GAC, COSPE)
```

```{r}
ggplot(data = COSPEC) +  geom_polygon(data = GAC, aes(x = long, y = lat, group = group), color ="gray50", fill = "white") + geom_polygon(aes(x = long, y = lat, fill = Freq, group = group), color = "azure4") + coord_fixed(1.3)  + scale_fill_gradient(high = "purple", low = "white", na.value="azure", trans = "log10") + theme_map() + theme( 
  axis.text = element_blank(),
  axis.line = element_blank(),
  axis.ticks = element_blank(),
  panel.border = element_blank(),
  panel.grid = element_blank(),
  axis.title = element_blank(), legend.position = "right") + facet_wrap( ~ Species)
```
```{r}
RACMAP <- with(CDV_Clean, table(County, Species, CollectionYear))
...
ggplot(data = RMP) +  geom_polygon(data = GAC, aes(x = long, y = lat, group = group), color ="gray50", fill = "white") + geom_polygon(aes(x = long, y = lat, fill = Freq, group = group), color = "azure4") + coord_fixed(1.3)  + scale_fill_gradient(high = "purple", low = "white", na.value="azure", trans = "log10") + theme_map() + theme( 
  ...
  axis.title = element_blank(), legend.position = "right") + facet_wrap( ~ CollectionYear)
```
#can i get these 2 overtime?, add pop centres to map