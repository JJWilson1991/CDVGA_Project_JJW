...
CDV_Clean19 <-   readRDS(here("data","processed_data","CDV19_clean.rds"))
...
ggsave(here("results", "Figures_new", "Cases_per_State19.png"),plot = SEC4)
...
statetable %>% saveRDS(here("results", "Tables", "statetable19.rds"))
...
ggsave(here("results", "Figures_new", "Cases_per_State_WrapYear19.png"),plot = SECT4)
...
sepSE_sf<-sepSE_sf%>% mutate(State = recode(State, "North carolina" = "North Carolina", "South carolina" = "South Carolina", "West virginia"= "West Virginia"))
 
#calculate the centroids for each county

SE_centroid <- st_centroid(sepSE_sf)
colnames(SE_centroid)[4]<- "Centroid"



SEtest4 <-  SE_centroid%>% mutate(tosplit= as.character(Centroid))
...
SE_centroid_split %>% saveRDS(here("data", "processed_data", "SE_centroid.rds"))
...
fil_spec_19<-subset(cent_spec_19, State%in% "Georgia")
RGFfil_spec_19<-as_data_frame(subset(fil_spec_19, Species%in% c("Raccoon", "Gray Fox")))

#redo everything for plotting points

GApoint<-subset(cent_SE_19, State%in% "Georgia")
RGFfil_spec_19<- RGFfil_spec_19[-c(10,11)]
...
ggsave(here("results", "Figures_new", "All_County_SF19.png"),plot = SF_County19)
...
ggsave(here("results", "Figures_new", "GA_County_SF19.png"),plot = GA_County19SF)
...
presenceSF_County19 <- ggplot() + 
  ...
  labs(fill= "Presence") +  scale_fill_manual(values = c('white', 'turquoise4')) #+facet_wrap(~State)
# #+ 
#   geom_point(data = GAcitiesspread, aes(x = long, y = lat, size = Population), alpha = 0.5) + 
#   geom_text(data = GAcitiesspread, aes(x = long, y = lat, label = name, group = NULL), position = position_jitter(width=0.3,height=0.3)) 
...
g3 <- g2 + 
  geom_point(data = GAcitiesspread, aes(x = long, y = lat), alpha = 0.5) + 
  geom_text(data = GAcitiesspread, aes(x = long, y = lat, label = name, group = NULL), position = position_jitter(width=0.3,height=0.3)) 
g3
...
```{r}
#plot the cases per countiy for each species
...
SP2 <- SP1 + 
  scale_fill_gradient(high = "purple", low = "white", na.value="white", trans = "log10") + 
  theme_map() + 
  theme( 
  axis.text = element_blank(),
  axis.line = element_blank(),
...
```
```{r}
#plot the raccon cases in each coutny for ewach year
...
```
```{r}
#repoeat the above process for grayfoxes
...
ggsave(here("results", "Figures", "Presence_WrapYear.png"),plot = Presence, height = 7, width = 7)
```

```{r}
#plot animation of county presence over time
...
```
```{r}
#The top county map is wrong, lets try with simple feature
...
```{r}
#split the centroid data into lattitude and longtiude as well as the point data
...
```
```{r}
#species plotted as different colored points for each year
...
ggsave(here("results", "Figures", "Species_County_stationary.png"), plot=k4, height = 7, width = 7)
```
```{r}

#Re do plotly mapping over time but in this case use the centroid data to plot raccoon and gray fox cases seperately using jitter so they are offset form each other, may show different species patterns
...
w1<- k2+ stat_density2d(data=centrLL, aes(x=long, y=lat, fill=..level.., alpha=..level..), 
 ...
 ggsave(here("results", "Figures", "RGF_case_map_smoothed.png"), plot = w1)
```
```{r}
#is it possible to use colour to seprate the cases per year on the map
ll3 <- g2 + 
 ...
 #there is still a lot of overlap and its difficult to read into this
...
RAw_GA_col <- Raw_Census_GA[c(2,3)]
...
testing3
#as atlanta is so densely populated its probably best to use a log scale
```
```{r}
log2 <- testing1 + 
  theme( 
...
RAw_Area_col <- Raw_Area_GA[c(3,4,5,6,7,8,9,10)]
...
```
```{r}
#plot species cases on a background of populati9n density per state
logDT3 <- DT2 + 
  theme( 
  axis.text = element_blank(),
  axis.line = element_blank(),
...
```{r}
 #plot ,map of cases per area with popualtion centres
ds<- CD1caseperpop[,c(1,10)]
dens<- left_join(cotr, ds, by= "County") 

DES1 <- ggplot(data = dens) +  
  geom_polygon(aes(x = long, y = lat, fill = Cases_per_area, group = group), color = "azure4") + 
  coord_fixed(1.3)  + 
  scale_fill_gradient(high = "magenta4", low = "white", na.value="white", trans = "log10") + 
  theme_map() 
...
DES2 <- DES1 + 
  ...
  panel.border = element_blank(),
  panel.grid = element_blank(),
  axis.title = element_blank(), legend.position = "right")  + labs(fill= "Cases/Area") 

DES3 <- DES2 + 
  geom_point(data = GAcitiesspread, aes(x = long, y = lat, size = 2), alpha = 0.5) + 
  geom_text(data = GAcitiesspread, aes(x = long, y = lat, label = name, group = NULL), position = position_jitter(width=0.3,height=0.3)) 
DES3

ggsave(here("results", "Figures", "Cases_per_area_map.png"), plot = DES3)