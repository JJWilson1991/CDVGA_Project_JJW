...
GAspecyear19<- ggplot() + geom_sf(data=Georgia_coorod, lwd=0.1, aes(geometry=geometry)) +  geom_point(data = pointRGF19_wrap, aes(x = long, y = lat, color= Species), alpha = 0.5, size=0.8,  position = position_jitter(width=0.05,height=0.05 ), na.rm = T) + scale_colour_discrete( na.translate=F ) +facet_wrap(~CollectionYear) + theme_map() + theme(legend.position="right")
GAspecyear19
...
geom_sf(data=Georgia_coorod, lwd=0.1, aes(geometry=geometry), fill="white")+  geom_sf(data = GCY2, lwd=0.1, aes(geometry=geometry, fill =Presence))  +
