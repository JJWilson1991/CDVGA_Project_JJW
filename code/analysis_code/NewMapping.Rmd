...
GAspecyear19<- ggplot() + geom_sf(data=Georgia_coorod, lwd=0.1, aes(geometry=geom)) +  geom_point(data = pointRGF19_wrap, aes(x = long, y = lat, color= Species), alpha = 0.5, size=0.8,  position = position_jitter(width=0.05,height=0.05 ), na.rm = T) + scale_colour_discrete( na.translate=F ) +facet_wrap(~CollectionYear) + theme_map() + theme(legend.position="right")
GAspecyear19
...
Spec_dens_logpop<-ggplot(subCounty_Dens_freq, aes(log10(Pop_density), Cases_year_area, color = Species,  group=Species)) + geom_point(aes(shape=Species))+geom_smooth() + labs(title = "Population density of conties reporting CDv cases in a given year and the number of cases per square mile in the county in Georgia", x="log10(Human Population Density per sqm)", y="Cases per sqm")  + scale_color_manual(values = c("blue", "darkgreen"), na.translate=F)
...
Av_pop_den_cases<-ggplot(Av_pop_spec) +geom_sf(aes(geometry=geom, fill=Mean_Pop_density)) + scale_fill_gradient(low = "beige", high = "chocolate4", trans= "log10") +geom_point(data =Av_pop_spec, aes(x = long, y = lat, color= Species), alpha = 0.5, position = position_jitter(width=0.05,height=0.05 ), na.rm = T) + scale_colour_discrete( na.translate=F )
...
av_pop_cases%<>% dplyr::mutate(Cases_per_area= Freq/Land_Area_sqm)

avLogpopdens_cases<- ggplot(av_pop_cases , aes(log10(Mean_Pop_density), Cases_per_area)) +geom_point() + geom_smooth() + labs(x="log10(Population Density per sqm)", y="Cases per sqm")
avLogpopdens_cases
