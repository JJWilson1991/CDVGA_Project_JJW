...
vis_miss(TRawCDVCol)
#not bad, the only variable which looks like it may be somewhat problematic is Area
```
```{r}
...
#the sex column is a little bit messy but should be easily fixed, observations like "M,F" or "Both, may be linked to the two species observation earlier
```
```{r}
...
#there looks to be what may be spelling mistakes here or they may be counties in different states with different spelling, shall have to look deeper
...
numberperagegroup
#so a lot of these are single obsv so we can probably group them into the larger groups
